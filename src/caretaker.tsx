...
function EventPublisher({ needs, busy, onPublish }: { needs: CareNeed[]; busy: boolean; onPublish: (body: Record<string, unknown>) => Promise<void> }) {
  return <form className="publish-card" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); onPublish({ action: 'publish_event', eventType: form.get('eventType'), catId: form.get('catId'), needId: form.get('needId'), title: form.get('title'), detail: form.get('detail'), worldArtifact: form.get('worldArtifact'), public: form.get('public') === 'on' }).then(() => formElement.reset()) }}><Video /><p className="care-eyebrow">04 · FIELD DISPATCH</p><h2>Record what happened.</h2><label>Type<select name="eventType"><option value="care.completed">Care completed</option><option value="dispatch.published">Dispatch published</option><option value="need.updated">Need updated</option><option value="cat.updated">Cat update</option><option value="funds.allocated">Funds allocated</option></select></label><div className="care-form-row"><label>Cat<select name="catId"><option value="">Sanctuary-wide</option><option value="splotch">Splotch</option><option value="mabel">Mabel</option><option value="winona-p-gray">Winona P. Gray</option><option value="gray-power">Gray Power</option><option value="gabriel">Gabriel</option><option value="poly">Poly</option></select></label><label>Need<select name="needId"><option value="">None</option>{needs.map((need) => <option value={need.need_id} key={need.need_id}>{need.title}</option>)}</select></label></div><label>Title<input name="title" required maxLength={160} /></label><label>What happened<textarea name="detail" rows={4} required maxLength={1600} /></label><label>World artifact key<input name="worldArtifact" placeholder="Optional · e.g. splotch-field-update" /></label><label className="care-check"><input type="checkbox" name="public" /> Publish to the player’s reality thread</label><button disabled={busy}><Send size={15} /> Record event</button></form>
}
